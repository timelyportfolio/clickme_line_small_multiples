...
var data = {{ get_data_as_json(opts) }};
        ...
        series.forEach(function (serie) {
                serie.max=d3.max(serie.values,function(d) {return +d.total});
                seie.min=d3.min(serie.values,function(d) {return +d.total});
        })
...
function drawSmallLineChart(series, container,max) {
        ...
        var x = d3.scale.linear().domain([0,series[0].values.length]).range([0,w]);
        ...
        var xAxis = d3.svg.axis().orient("bottom").ticks(5);//.tickFormat(function(d) {return dateFormatter(series[0].values[d].date)}).ticks(3);
