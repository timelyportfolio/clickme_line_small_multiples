...
<body>
  ...
  var data = {{ get_data_as_json(opts) }};
          ...
          series = d3.nest()
                 ...
                 .sortvalue(function(a,b) {return a.date<b.date?-1:(a.date==b.date)?0:1})
                 .entries(data);
  
          series.forEach(function (serie) {
                  serie.max=d3.max(serie.value,function(d) {return +d.total});
                  seie.min=d3.min(serie.value,function(d) {return +d.total});
          })
  ...
  function drawSmallLineChart(series, container,max) {
          ...
          var x = d3.scale.linear().domain([0,series[0].value.length]).range([0,w]);
          ...
          var xAxis = d3.svg.axis().orient("bottom").ticks(5);//.tickFormat(function(d) {return dateFormatter(series[0].value[d].date)}).ticks(3);
